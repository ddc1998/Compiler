...
%token <node> STRING AND OR Node_TOKEN PRINTINT PRINTREAL PRINTSTRING
...
prog : PROGRAM id LPAREN id_list RPAREN SEMICOLON
  ...
  DOT { 
            ...
            AdoptChildren($$, $4);
            AdoptChildren($$, $7);
            AdoptChildren($$, MakeNode(Node_FUNC_DECL));
            AdoptChildren($$, $8);
            AdoptChildren($$, MakeNode(Node_MAIN_FUNC));
            AdoptChildren($$, $9);
            AdoptChildren($$, MakeNode(Node_DOT));
        }
 ;
...
compound_statement : 
    PBEGIN statement_list SEMICOLON END {
...
procedure_statement : 
 ...
 |  PRINTINT LPAREN expression_list RPAREN {
            $$ = MakeNode(Node_PRINTINT);
            AdoptChildren($$, $3);
        }
 |  PRINTREAL LPAREN expression_list RPAREN {
            $$ = MakeNode(Node_PRINTREAL);
            AdoptChildren($$, $3);
        }
 |  PRINTSTRING LPAREN expression_list RPAREN {
     printf("test\n");
            $$ = MakeNode(Node_PRINTSTRING);
            AdoptChildren($$, $3);
        }
 ;
